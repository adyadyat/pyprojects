...
profitability = (finance[y][m]["income"] - finance[y][m]["expenses"]) / finance[y][m]["income"] * 100

print("{:.0f}%".format(float(profitability)))
"""
РЕНТАБЕЛЬНОСТЬ
Рентабельность продаж рассчитывается по следующей формуле: 
(доходы − расходы) / доходы x 100.
В редакторе ниже находится словарь finance, 
который содержит данные по доходам (income) 
и расходам (expenses) за все месяцы 2019 и 2020 годов.

Напишите программу, которая получает из аргументов 
командной строки два параметра: год и месяц, 
а затем выводит рентабельность продаж за этот период.
